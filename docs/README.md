...
- [Client package observer runtime plan](planning/active/client_package_observer_runtime_plan.md) is the active plan for the shared rich-client observer runtime in `packages/client`; PRs 1-3 (extraction, reliability deltas, Station's read-only observer overlay) are implemented, and the remaining scope is PR 4 Station command dispatch.
- [Station TUI overlay seams review](planning/active/station_tui_overlay_seams_review.md) is the active review of the Station WOSM overlay, the copied `apps/tui` dashboard core, and the extraction path toward a shared render-free dashboard package.
- [Station dashboard target architecture](planning/active/station_dashboard_core_target_architecture.md) is the active plan adopting that review's judgment: the dashboard-core end state, the ordered path to it (guardrails, then extraction, then Station command dispatch), and the PR #78 review findings ledger.
...
- [WOSM Station spike](planning/active/wosm_station_spike.md) is the active spike plan for testing a full-screen WOSM-owned workspace that hosts real PTY-backed terminal panes without replacing tmux/classic mode; its Phased Plan And Status section tracks what has landed and what is next.
