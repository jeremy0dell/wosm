...
Status: implemented in `apps/tui`.

Implementation note: the TUI now uses `apps/tui/src/state/` for screen-owned key
transitions, command builders, and a scoped Zustand store. The existing `C`
project-collapse workflow is modeled as a `projectCollapse` screen so the
pre-existing UX stays intact while removing the generic prompt/mode layer.
