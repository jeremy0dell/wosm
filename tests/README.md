...
- `integrations/terminal/tmux/test/integration/popup-real.test.ts` is an opt-in real tmux lane. Run it only with `WOSM_REAL_TMUX=1` and an installed `tmux`, optionally setting `WOSM_TMUX_BIN=/path/to/tmux`. It uses an isolated tmux server label and does not touch the normal wosm workbench.
- `tests/agent/real/claude/` is an opt-in real Claude lane. Run it only with `WOSM_REAL_CLAUDE=1`, a logged-in `claude`, and installed `tmux`, optionally setting `WOSM_CLAUDE_BIN=/path/to/claude` and `WOSM_TMUX_BIN=/path/to/tmux`. It is intentionally not part of `test:all`.
...
- `tests/agent/real/cursor/` is an opt-in real Cursor lane. Run it only with `WOSM_REAL_CURSOR=1`, `agent --version` passing, and installed `tmux`, optionally setting `WOSM_CURSOR_AGENT_BIN=/path/to/agent` and `WOSM_TMUX_BIN=/path/to/tmux`. It is intentionally not part of `test:all`.
- `tests/e2e/real/` is the product real E2E lane. Run it only with `WOSM_REAL_E2E=1`, `WOSM_REAL_WORKTRUNK=1`, `WOSM_REAL_CODEX=1`, real `wt`, real `tmux`, real `codex`, and a built `bin/wosm`. Use `pnpm test:e2e:real`, `pnpm test:e2e:real:local`, or focused scripts such as `pnpm test:e2e:real:codex-hooks`. It creates real Worktrunk/tmux/Codex sessions, opens the TUI in a real tmux popup for popup-navigation coverage, and includes a temp project-local Codex hook that calls `wosm-ingress codex`. It is intentionally excluded from `test:e2e` and `test:all`.
