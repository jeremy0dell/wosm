// ADAPTED-EXTRACTION from apps/tui/src/components/Dashboard/Dashboard.tsx and
// apps/tui/src/App/App.tsx (see ../../PROVENANCE.md): the pure string/layout
// content the dashboard chrome renders — header line composition with
// responsive widget/status dropping, footer labels, project header/empty-row
// labels, the viewport-item -> row-grid-input mapping, loading-body copy,
// and the screen-derived prompt/modal facts. Verbatim bodies; only the Ink
// components around them were left behind.
...
export function snapshotLoadingLines(
  ...
  if (observerConnectionStatus.state === "reconnecting") {
    return [
      ...
      { text: "retrying connection", color: "gray" },
      { text: " " },
      { text: "The dashboard will appear when the observer is ready.", color: "gray" },
    ];
  }
  ...
  if (!loading) {
    return [
      { text: " " },
      { text: "observer snapshot unavailable" },
      { text: "Check the error details and try refreshing when ready.", color: "gray" },
    ];
  }

  return [{ text: "Loading observer snapshot...", color: "gray" }];
}
...
/**
 * From apps/tui/src/components/CommandPrompt/CommandPrompt.tsx: the prompt
 * line per screen (the special-cased rename-slot and remove-confirm lines
 * plus textPromptForScreen below), flattened to text+color so the view only
 * renders. Lives beside commandPromptRows, which guards the same screens.
 */
